...
class BotFormController {
  ...
  BotFormController({String name = '', String weight = '15kg'})
    : nameCtrl = TextEditingController(text: name),
      weight = BotCategory.normalize(weight);
  ...
  void initState() {
    ...
    if (selectedTeam != null && selectedTeam.bots.isNotEmpty) {
      for (var bot in selectedTeam.bots) {
        _botControllers.add(
          BotFormController(name: bot.name, weight: bot.weight),
        );
      }
    } else {
                            ...
                            : () async {
                                final confirmed = await showDialog<bool>(
                                  ...
                                  builder: (dialogContext) => AlertDialog(
                                    ...
                                    content: Text(
                                      'Delete ${state.selectedTeam!.name}? '
                                      'Assigned participants will be unlinked.',
                                    ),
                                    actions: [
                                      TextButton(
                                        onPressed: () =>
                                ...
                                await ref
                                    .read(teamEditorViewModelProvider.notifier)
                                    .deleteTeam(state.selectedTeam!.id);
